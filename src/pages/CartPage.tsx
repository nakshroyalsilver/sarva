...
const CartPage = () => {
  ...
  // 3. Logic for Suggested Products (Filter out items already in cart)
  ...
  // Mock Logic: If coupon is "NEW10", give 10% off, else flat 500 if total > 0
                       ...
                       <div className="p-4 pt-0">
                         {isCouponApplied ? (
                           <div className="flex items-center justify-between bg-green-50 text-green-700 px-3 py-2 rounded border border-green-200 text-sm">
                             ...
                             <button onClick={() => { setIsCouponApplied(false); setCouponCode(""); }} className="text-xs underline hover:text-green-900">Remove</button>
                           </div>
                         ) : (
                           <div className="flex gap-2">
                             <input 
                               ...
                               className="flex-1 border border-gray-300 rounded px-3 py-2 text-sm outline-none focus:border-rose-500 uppercase placeholder:normal-case"
                             />
                             <button 
                               onClick={handleApplyCoupon}
                               disabled={!couponCode}
                               className="bg-gray-900 text-white px-4 py-2 rounded text-xs font-bold uppercase disabled:bg-gray-300 disabled:cursor-not-allowed hover:bg-gray-700 transition-colors"
                             >
                               Apply
                             </button>
                           </div>
                 ...
                 <AnimatePresence>
                   {isGiftOpen && (
                     ...
                     >
                       <div className="p-4 pt-0">
                         <textarea 
                           placeholder="Type your message here... (We will print this on a beautiful card)"
                           maxLength={200}
                           rows={3}
                           value={giftMessage}
                           onChange={(e) => setGiftMessage(e.target.value)}
                           className="w-full border border-gray-300 rounded p-3 text-sm outline-none focus:border-rose-500 resize-none"
                         />
